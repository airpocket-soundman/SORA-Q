...
#define CONSOLE_BAUDRATE    115200 //57600
...
void splitString(String input, String &var1, String &var2, String &var3) {
    int startIndex = 0;
    int commaIndex;
    int varCount = 0;

    while ((commaIndex = input.indexOf(',', startIndex)) != -1 && varCount < 2) {
        if (varCount == 0) {
    ...
    // 最後の変数に残りの部分を格納
    if (varCount < 2) {
        var3 = input.substring(startIndex);
    } else {
        var3 = ""; // 余った部分がない場合
    }
}
...
void checkMQTTtopic(){ 
  ...
  while (gs2200.available()) {
    ...
    // dataをパース
    splitString(data, param1, param2, param3);
