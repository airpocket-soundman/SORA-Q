...
static String const label[2]= {"not detected","detected!"};
...
void CamCB(CamImage img) {
  ...
  // text描画
  float probability = output[0]; // 出力の最初の要素を確率として取得

  // しきい値を使用してクラスを判断
  if (probability >= 0.5) { // バイナリ分類のしきい値は0.5
    gStrResult = String(label[1]) + String(":") + String(probability); // 陽性クラスの場合
  } else {
    gStrResult = String(label[0]) + String(":") + String(probability); // 陰性クラスの場合
  }
  Serial.println(gStrResult);
  putStringOnLcd(gStrResult, ILI9341_YELLOW);

  // モード変更有無をチェック
  if (probability >= 0.5 && before_value == 0) {
    ChangeModeFlag = true; // 陽性から陰性への変更
  } else if (probability < 0.5 && before_value == 1) {
    ChangeModeFlag = true; // 陰性から陽性への変更
  }

  // 更新
  before_value = (probability >= 0.5) ? 1 : 0; // 陽性クラスは1、陰性クラスは0
  // flag初期化
